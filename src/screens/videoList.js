...
export default class VideoList extends Component {
  ...
  getData() {
    console.log("Here in video page");
    ...
    if (this.state.page == 'one') {
      ...
      var url = 'https://www.googleapis.com/youtube/v3/search?key=AIzaSyCCHuayCrwwcRAUZ__zTYyOP-ax5FD4R9E&channelId=UCswIOlMY2_DT05glwBsxZyg&part=snippet,id&order=date&maxResults=20'
      console.log(url)
      return fetch(url)
        .then((response) => response.json())
        .then((responseJson) => {         
          console.log("normal video"+responseJson.items);
      ...
      const { params } = this.props.navigation.state;
      var url = 'https://www.googleapis.com/youtube/v3/search?key=AIzaSyCCHuayCrwwcRAUZ__zTYyOP-ax5FD4R9E&channelId=UCswIOlMY2_DT05glwBsxZyg&part=snippet,id&order=date&maxResults=20&pageToken=' + this.state.pageToken
     
      return fetch('https://www.googleapis.com/youtube/v3/search?key=AIzaSyCCHuayCrwwcRAUZ__zTYyOP-ax5FD4R9E&channelId=UCswIOlMY2_DT05glwBsxZyg&part=snippet,id&order=date&maxResults=20&pageToken=' + this.state.pageToken)
        .then((response) => response.json())
        .then((responseJson) => {
  ...
  _renderItem = (itemData) => {
    try {                                 
      ...
      if (itemData.item.snippet.thumbnails) {
        console.log(itemData.item.snippet.liveBroadcastContent)
        if(itemData.item.snippet.liveBroadcastContent == 'none'){
          console.log("here")
        return (
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                NetInfo
                  ...
                  .then(isConnected => {
                    if (isConnected) {
                      navigate("VideoPlay", { videoid: itemData.item.id.videoId })
                    } else {
                      Toast.show('Oops no internet connection !', Toast.SHORT);
                    }
                  });                     
